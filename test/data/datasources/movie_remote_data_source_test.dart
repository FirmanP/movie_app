...
void main() {
  ...
  late MockSSLHttp mockSslHttp;
  late MockIOClient mockIOClient;

  setUp(() {
    mockHttpClient = MockHttpClient();
    dataSource = MovieRemoteDataSourceImpl(
        client: mockHttpClient, sslHttp: MockSSLHttp());
    mockSslHttp = MockSSLHttp();
  ...
  group('get Now Playing Movies', () {
    ...
    test('should return list of Movie Model when the response code is 200',
        () async {
      when(mockSslHttp.ioClientGet('$BASE_URL/movie/now_playing?$API_KEY'))
          .thenAnswer((_) async =>
              http.Response(readJson('dummy_data/now_playing.json'), 200));
      // arrange
      // ioc.IOClient ioClient = ioc.IOClient(await mockSslHttp.getCertificate);
      // ioClient.get(Uri.parse('$BASE_URL/movie/now_playing?$API_KEY')))
